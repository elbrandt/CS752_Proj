 ...
 *---------------------------------------------------------------------------
 * $File: //depot/povray/3.5/source/frontend/messageoutput.cpp $
 * $Revision: #17 $
 * $Change: 2929 $
...
void MessageOutput::RenderOptions(POVMSObjectPtr msg, POVMSObjectPtr, int)
{
	...
	if(b == true)
	{
		...
		(void)POVMSUtil_GetBool(msg, kPOVAttrib_Jitter, &b);
		...
		if(b == true)
			...
			Printf(RENDER_STREAM, "  Antialiasing.........On  (Method %d, Threshold %.3f, Depth %d, Jitter Off)\n",
...
void MessageOutput::RenderStatistics(POVMSObjectPtr msg, POVMSObjectPtr, int)
{
	...
	if(POVMSLongToCDouble(l) > 0.5)
		Printf(STATISTIC_STREAM, "Pixels:  %15.0f   Samples: %15.0f\n",
	...
	Printf(STATISTIC_STREAM, "----------------------------------------------------------------------------\n");
	Printf(STATISTIC_STREAM, "Ray->Shape Intersection          Tests       Succeeded\n");
	Printf(STATISTIC_STREAM, "----------------------------------------------------------------------------\n");
		...
		if(POVMSAttrList_Count(&attr, &cnt) == kNoErr)
		{
			...
			for(ii = 1; ii <= cnt; ii++)
			{
				if(POVMSAttrList_GetNth(&attr, ii, &obj) == kNoErr)
				{
					...
					if(POVMSLongToCDouble(l) > 0.5)
					{
						Printf(STATISTIC_STREAM, "%-22s  %14.0f  %14.0f\n", str,
						              POVMSLongToCDouble(l), POVMSLongToCDouble(l2));
					}
	...
	if(POVMSLongToCDouble(l) > 0.5)
	{
		...
		Printf(STATISTIC_STREAM, "Media Intervals:    %15.0f   Media Samples:   %15.0f\n",
		              POVMSLongToCDouble(l), POVMSLongToCDouble(l2));
	}
	...
	{
		Printf(STATISTIC_STREAM, "----------------------------------------------------------------------------\n");
		Printf(STATISTIC_STREAM, "Radiosity samples calculated:  %15d\n", i);
...
// FIXME		if(opts.Options & VERBOSE)
		{
			...
			else if((POVMSUtil_GetLong(msg, kPOVAttrib_CurrentToken, &ll) == kNoErr) && (ll > 0))
			{
				...
				Printf(STATUS_STREAM, " %ldK tokens", long(ll)/1000);
#endif /* SPEC_CPU */
			}
			// rendering progress
